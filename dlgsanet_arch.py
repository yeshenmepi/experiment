...
class ISA(nn.Module):
    ...
    def forward(self, x):
        ...
        local_features = local_features.permute(0, 2, 4, 1, 3, 5).contiguous().view(-1, C, self.patch_size,
                                                                                    self.patch_size)
        b,c,h,w = local_features.size()
        ...
        out = out.view(B, H // self.patch_size, W // self.patch_size, C, self.patch_size,
                                             self.patch_size)
...
class DLGSANet(nn.Module):
    ...
    def forward(self, x):
        ...
        return x[:, :, 0:old_h*self.upscale, 0:old_w*self.upscale]
